...
// List of specific personnel to display
...
export default async function FormantFormationPersonnelPage() {
  ...
  try {
    ...
    // Grouping logic (same as admin page)
    ...
    if (generalCoordinator) {
      groups['General Formation Coordinator'] = { title: 'General Formation Coordinator', members: [generalCoordinator] };
    }
    fetchedMembers.forEach(member => {
      if (member.title.rendered === coordinatorName) return;
      const province = getProvinceName(member) || 'Other Personnel';
      // --- TEMPORARY LOGGING ---
