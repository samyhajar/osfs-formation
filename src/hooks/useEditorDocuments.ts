...
export function useEditorDocuments(filters: PageFilterState, authLoading: boolean) {
  ...
  useEffect(() => {
    ...
    const fetchDocuments = async () => {
      console.log("useEditorDocuments: Fetching documents...");
      try {
        setLoading(true);
        setError(null);
        ...
        console.log("useEditorDocuments: Executing Supabase query...");
        const { data, error: fetchError } = await query;

        if (fetchError) {
          console.error("useEditorDocuments: Fetch error:", fetchError);
          setError(`Failed to fetch documents: ${fetchError.message}`);
          setDocuments([]);
        } else {
          console.log("useEditorDocuments: Documents fetched successfully:", data?.length || 0);
          if (!data || !Array.isArray(data)) {
            console.error("useEditorDocuments: Invalid data returned:", data);
        ...
        console.error("useEditorDocuments: Unexpected error:", err);
        setError(`Unexpected error: ${err instanceof Error ? err.message : String(err)}`);
        setDocuments([]);
      } finally {
        setLoading(false);
        console.log("useEditorDocuments: fetchDocuments finished. Loading:", false);
      }
    };
  ...
  useEffect(() => {
    const fetchCategoryCounts = async () => {
      ...
      if (rpcError) {
        console.error("useEditorDocuments: Error fetching category counts via RPC:", rpcError);
        ...
        console.log("useEditorDocuments: Category counts fetched:", counts);
